...
*   **Resolution**: Updated `frontend/nginx.conf` to use `location ^~ /uploads/` which forces Nginx to prioritize this path over other rules.
*   **Verification**: Verified access to uploaded images returns `200 OK`.

### 3. HTTPS Enabled
*   **Issue**: User requested secure access via HTTPS.
*   **Resolution**:
    *   Used Certbot to generate Let's Encrypt certificates for `bewathiq.com`.
    *   Updated Nginx to serve traffic on port 443 using SSL.
    *   Enforced automatic redirection from HTTP to HTTPS.
*   **Verification**: Verified `https://bewathiq.com` loads securely and `http://` redirects to `https://`.

## Current Status
*   **Backend**: Running and healthy. Permissions seeded.
*   **Frontend**: Running securely via HTTPS.
*   **Database**: Migrations up to date.
*   **Images**: Accessible and secure.
*   **URL**: [https://bewathiq.com](https://bewathiq.com)
