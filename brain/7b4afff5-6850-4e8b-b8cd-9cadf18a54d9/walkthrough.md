...
- `generateAndSendOTP()` — creates 6-digit code, bcrypt-hashes it, stores in DB, sends via `EmailService`
- `verifyOTP()` — validates code, enforces 5-attempt limit, activates tenant on success
- `resendOTP()` — 60-second cooldown, deletes old token, sends new one
...
- `register()` — creates tenant/user as **inactive** (`PENDING_EMAIL`), sends OTP, returns `{ requiresVerification: true }` instead of JWT
- `verifyEmail()` — **new method** that activates tenant+user and returns JWT for auto-login
- `login()` — detects `PENDING_EMAIL` tenants and redirects to verification
...
- Added `registrationStatus` and `emailVerifiedAt` to `Tenant`
...
- `useRegister` redirects to `/verify-email` on success
- `useLogin` detects `requiresVerification` and redirects
