...
- **`askAI(query, tenantId, userId)`** — Main entry point
  1. Check cache (existing `CacheService`)
...
- **`getUsageStats(tenantId, userId)`** — Monthly usage dashboard
- **`checkQuota(tenantId)`** — Verify AI quota not exceeded

Uses existing `CacheService` for response caching (7-day TTL for keyword, 30-day for AI).
...
- Replace stub `aiSearch()` with call to new `LegalAIService`
